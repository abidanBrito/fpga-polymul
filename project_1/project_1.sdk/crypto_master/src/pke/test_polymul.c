...
void test_polymul(){
	...
	uint64_t result[256], poly_a[256], poly_b[256], result_sw[256];

	uint64_t hw_time_sum = 0;
	uint64_t sw_time_sum = 0;
	for (int i = 0; i < N_TESTS; i++) {
		...
		XTime_GetTime(&tEnd);
		hw_time_sum += 2*(tEnd-tStart);

	    XTime_GetTime(&tStart);
	    polymul(result_sw, poly_a, poly_b);
		XTime_GetTime(&tEnd);
		sw_time_sum += 2*(tEnd-tStart);

	    for (int i = 0; i < 256; i++) {
			if (result[i] != result_sw[i]) {
				printf("Test failed %d %llx %llx\n", i, result[i], result_sw[i]);
				return;
			}
	    }
	}
