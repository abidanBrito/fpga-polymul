...
void init_INS_Poly_mult(unsigned long long *INS, unsigned int INP_ADDRESS1, unsigned int INP_ADDRESS2, unsigned int OUT_ADDRESS)
{
	int i;
		...
		unsigned long long program_word_5 =  ((unsigned long long) 1<<35) + ((unsigned long long) OUT_ADDRESS<<25) + (768<<15)+(OUT_ADDRESS<<5) + 26;
		...
		ins_group[3] = program_word_2;
		ins_group[5] = program_word_3;
		ins_group[7] = program_word_4;
		ins_group[9] = program_word_5;
		for(i=0; i<64; i++)
			INS[i] = ins_group[i];
}
